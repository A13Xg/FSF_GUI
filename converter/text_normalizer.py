...
class TextNormalizer:
    ...
    def normalize_text(cls, text: str) -> str:
        ...
        if text != original_text:
            logger.info(f"Text normalization applied: '{original_text[:50]}...' -> '{text[:50]}...'")
    ...
    def sanitize_for_compendium_lookup(cls, name: str) -> str:
        ...
        while preserving the essential text content.
    ...
    @classmethod
    def get_text_difference_summary(cls, original: str, normalized: str) -> Dict[str, any]:
...
def test_text_normalization():
    ...
    test_cases = [
        ...
        ('Text with\ufffdreplacement', 'Text withreplacement', 'Replacement character removal'),
    ]
    ...
    for input_text, expected, description in test_cases:
        ...
        print(f"{status} {description}: '{input_text}' -> '{result}' (expected: '{expected}')")
        if result != expected:
