...
GITHUB_BRANCH = "main"
GITHUB_RAW_URL = f"https://raw.githubusercontent.com/{GITHUB_REPO}/{GITHUB_BRANCH}/src/packs"
...
def _fetch_github_files(verbose=False):
    ...
    try:
        ...
        req = urllib.request.Request(
            GITHUB_API_URL,
            headers={'Accept': 'application/vnd.github.v3+json', 'User-Agent': 'forgesteel-converter'}
        )
...
def _fetch_pack_files(api_url, items_dict, pack_name, verbose=False, depth=0):
    ...
    try:
        ...
        if verbose:
            print(f"DEBUG: Processing {len(items)} items in {pack_name} (depth {depth})")
        ...
        for item in items:
            if item['type'] == 'file' and item['name'].endswith('.json'):
                ...
                _fetch_pack_files(item['url'], items_dict, pack_name, verbose, depth + 1)
...
def load_compendium_items(compendium_path, verbose=False):
    """Loads all items from the compendium packs.
    
    Uses a hybrid approach:
    1. Local compendium (if available)
    2. Cache (if available)
    3. GitHub (with automatic caching)
    ...
    if compendium_path.exists() and compendium_path.is_dir():
        ...
        if items:
            ...
            if verbose:
                print(f"DEBUG: Compendium stats: {items_loaded} items loaded from local")
            return items
    ...
    if cache_dir.exists() and cache_dir.is_dir():
        ...
        if items:
            items_loaded = len(items)
            if verbose:
                print(f"DEBUG: Compendium stats: {items_loaded} items loaded from cache")
            return items
...
def _load_json_item(file_path, items_dict, verbose=False):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if (item_data.get('type') == 'ability' and
                'system' in item_data and
                'type' in item_data['system']):
            ...
            if '_dsid' in item_data.get('system', {}):
                ...
                if dsid not in items_dict:
                    ...
                    existing_category = items_dict[dsid].get('system', {}).get('category', '')
                    ...
                    if existing_category == 'heroic' and new_category != 'heroic':
                        if verbose:
                            print(f"DEBUG: Duplicate {dsid}: preferring {item_data.get('name')} (non-heroic) over existing (heroic)")
                        items_dict[dsid] = item_data
