...
class DescriptionTransfer:
    ...
    @classmethod
    def transfer_description(cls, source_item: Dict[str, Any],
                           compendium_item: Optional[Dict[str, Any]] = None) -> str:
        ...
        if compendium_item:
            ...
            if not compendium_desc:
                ...
                compendium_desc = compendium_item.get("system", {}).get("description", {}).get("value", "").strip()

            # Try effect.before field (used in ability items)
            if not compendium_desc:
                compendium_desc = compendium_item.get("system", {}).get("effect", {}).get("before", "").strip()

            if compendium_desc:
                logger.debug("Using compendium description")
                return cls._preserve_formatting(TextNormalizer.normalize_text(compendium_desc))
            else:
    ...
    def _preserve_formatting(cls, description: str) -> str:
        ...
        allowed_tags = ['p', 'strong', 'em', 'i', 'b', 'u', 'ul', 'ol', 'li', 'br', 'div', 'span']
        ...
        for tag in allowed_tags:
            ...
            self_closing_count = len(re.findall(self_closing_pattern, description, re.IGNORECASE))
            ...
            if tag not in ['br', 'img', 'hr']:  # These are self-closing tags
                ...
                if close_count != expected_close:
                    logger.warning(f"Mismatched {tag} tags in description: {open_count} open, {close_count} close, {self_closing_count} self-closing")
        ...
        for pattern in dangerous_patterns:
            ...
            if matches:
                logger.warning(f"Found potentially dangerous content in description: {pattern}")
    ...
    @classmethod
    def enhance_description_for_foundry(cls, description: str, item_type: str = "") -> str:
    ...
    @classmethod
    def audit_description_transfers(cls, source_items: list,
        ...
        for i, (source, converted) in enumerate(zip(source_items, converted_items)):
            ...
            converted_desc = converted.get("system", {}).get("description", {}).get("value", "")
            ...
            if source_desc and converted_desc and len(converted_desc) < len(source_desc) * 0.5:
...
def test_description_transfer():
    ...
    # Test enhancement
    enhanced = DescriptionTransfer.enhance_description_for_foundry("Simple text description", "ability")
